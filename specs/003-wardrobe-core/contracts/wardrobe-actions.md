...
- **输入**：无业务字段。
- **前置条件**：当前请求必须是有效体验身份，且账号没有 `demo_key IS NULL` 的真实衣物，内置演示衣橱尚未完整。正式账号、已有真实衣物或演示衣橱完整时不提供入口。
- **行为**：读取已有 `demo_key`，只为缺失项生成并上传私有 PNG，再插入缺失记录。
- **鉴权**：Action 内使用 `auth.getUser()` 重新确认当前 Auth 用户和匿名状态，并通过当前 `user_id` 查询真实衣物；不得信任页面显隐或客户端字段。
- **并发规则**：`(user_id, demo_key)` 唯一约束作为最终幂等保证；重复操作保持每个内置 `demo_key` 最多一条记录。
- **成功**：返回当前演示总数和本次补齐数量。
