...
assert.match(component, /brand-motion-part--top/);
assert.match(component, /brand-motion-part--middle/);
assert.match(component, /brand-motion-part--bottom/);
assert.match(component, /brand-motion-sheen/);
...
assert.match(
  brandCss,
  /mask-image:\s*url\("\/brand\/ensemble-icon-a-folded-e\.png"\)/,
);
for (const name of [
  "brand-splash-top",
  "brand-splash-middle",
  ...
  assert.match(brandCss, new RegExp(`@keyframes ${name}`));
assert.doesNotMatch(brandCss, /transition:\s*all/);
assert.doesNotMatch(brandCss, /scale\(0(?:[),\s])/);

assert.match(
  brandCss,
  /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.brand-motion-part[\s\S]*animation:\s*none/,
);
assert.match(
  brandCss,
...
if (process.argv.includes("--preview")) {
  ...
  const motion = (variant, label) =>
    `<div class="brand-motion" data-variant="${variant}" ${variant === "loader" ? 'role="status" aria-live="polite"' : ""}><span class="brand-motion-stage" aria-hidden="true"><span class="brand-motion-halo"></span><span class="brand-motion-mark">${image("brand-motion-mark-base")}${image("brand-motion-part brand-motion-part--top")}${image("brand-motion-part brand-motion-part--middle")}${image("brand-motion-part brand-motion-part--bottom")}<span class="brand-motion-sheen"></span></span></span>${variant === "splash" ? '<span class="brand-name-lockup brand-motion-name"><span class="brand-name-english">Ensemble</span><span class="brand-name-chinese">衣拍即合</span></span>' : `<span class="brand-motion-label">${label}</span>`}</div>`;
  const server = createServer(async (req, res) => {
    ...
    res.end(
      `<!doctype html><html class="${isDark ? "dark" : "light"}"><head><meta name="viewport" content="width=device-width,initial-scale=1">${stylesheets.map((name) => `<link rel="stylesheet" href="/styles/${name}">`).join("")}<style>.brand-motion-preview .brand-splash,.brand-motion-preview .brand-motion *{animation-play-state:paused!important;animation-delay:-560ms!important}</style></head><body class="${isLive ? "" : "brand-motion-preview"}"><main class="app-backdrop" style="min-height:100svh;max-width:480px;margin:auto">${isLoader ? `<div class="brand-page-loader">${motion("loader", "正在准备页面")}</div>` : `<div class="brand-splash">${motion("splash", "")}</div>`}</main><p style="position:fixed;left:12px;bottom:12px;z-index:200;font-size:11px;color:var(--text-secondary)">本地动效${isLive ? "实时" : "定格"}样本 · ${isLoader ? "加载" : "开屏中段"} · ${isDark ? "深色" : "浅色"}</p></body></html>`,
    );
  });
  server.listen(3013, "127.0.0.1", () =>
    console.log(
      "本地品牌动效样本：http://127.0.0.1:3013/ 与 /loader（?theme=dark 切换深色）",
    ),
  );
}
