...
先重排导航与视觉，再退役画布/抠图，最后切换每日推荐并增加诊断。增量范围继续完善推荐个性化：把反馈账本形成的风格分数和每日可信时尚资讯转换成最小模型上下文，并对模型与规则降级统一执行动态命名。最新导航增量将穿搭日记改为顶部左侧圆形入口，顶部保持日记/品牌/头像三点平衡；底部五项恢复推荐穿搭，中央添加改为倒角矩形。收藏继续作为日记内分页。沿用图片识别、天气、收藏和日记数据，保留历史资产与旧路由兼容。
...
- 顶部使用青柠色浮动圆角壳层，左侧为圆形日记、中央为品牌、右侧为圆形头像；底部使用浅紫浮动壳层，两者目标高度约 64-72px。底部五项为首页、衣橱、中央倒角矩形添加、推荐和资讯，标签统一两个汉字。
...
- components/bottom-navigation.tsx、status-header.tsx：首页/衣橱/中央倒角矩形添加/推荐/资讯五项 Dock，以及圆形日记/品牌/圆形头像顶栏。
