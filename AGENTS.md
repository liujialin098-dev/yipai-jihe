...
- SDD-030 最新本地导航决策：顶部青柠栏只保留品牌和右上圆形头像，头像是个人主页唯一顶部入口；底部使用浅紫材质并固定为首页、衣橱、中央添加、时尚资讯、日记五项。推荐由首页主操作承接；日记页合并日记、收藏、利用率，旧 `/favorites` 重定向到收藏栏。该增量尚未部署，不得写成线上已生效。
- 底部固定为 **首页 / 衣橱 / 添加衣物 / 时尚资讯 / 日记**，添加衣物在正中央且最突出；底栏使用克制的浅紫材质。推荐由首页主操作进入，日记内合并收藏与利用率。顶部青柠软渐变浮动圆角壳层只保留居中品牌和右上头像，个人主页不得恢复独立文字入口。上下栏视觉高度差保持在约 8px 内，四周保留背景间距；滚动时保持 sticky/fixed 完整可达，不得被正文、头像或标题覆盖。应用外壳使用 `overflow-x-clip`，不得恢复会破坏 sticky 的 `overflow-x-hidden`。
...
- `components/`：移动端应用外壳、统一品牌标志、青柠品牌/头像顶栏、浅紫五项底部导航、会话启动、衣橱筛选/卡片/表单、入库工作区、推荐换件、个人资料编辑、日记/收藏合并视图、偏好问卷和通用状态；`components/ui/` 保留 shadcn/ui 基础组件。
...
- 当前阶段：SDD-030 已上线基线为 `dpl_8F4H53QwBdMJoYmgfHgPqS6bNHb3` / `ddbbc6e`；顶部单排品牌头像、浅紫五项底栏及日记/收藏合并为本地已验收增量，尚未部署。真实百炼北京凭据联调仍未完成，因此状态保持“部分完成”。SDD-002 本人历史账号重登录、手机定位与联网仍待集中调试。不得代替用户输入或保存密码、擅改公开策略或保护绕过设置。每阶段开发后必须更新 progress.md，不得把固定测试、规则后备或本地成功标记为真实接口验收。
